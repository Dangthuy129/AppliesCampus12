...
public class BudgetDb extends SQLiteOpenHelper {
    ...
    public long addBudget(BudgetModel budget) {
        ...
        String query = "SELECT COUNT(*) FROM " + TABLE_BUDGET +
                " WHERE " + COLUMN_PRICE + " = ? AND " + COLUMN_DESCRIPTION + " = ?";
        Cursor cursor = db.rawQuery(query, new String[]{
                String.valueOf(budget.getPrice()), budget.getDescription()
        });
        ...
        if (cursor != null) {
            ...
            cursor.close();
            if (count > 0) {
                // Dữ liệu đã tồn tại, trả về -1
                return -1;
            }
        }

        // Nếu không trùng lặp, thêm dữ liệu mới
